...
// Attach the media URL only during a play gesture, including on iOS Safari.
function prepareCaseVideos() {
  dialogContent.querySelectorAll('[data-video-player]').forEach(player => {
    ...
    player.releaseVideo = () => {
      ...
      // Abort an outstanding download and release the decoder after closing.
      video.load();
    };
    const fail = () => {
      if (disposed) return;
      ...
      status.textContent = '影片暫時無法播放，請稍後重試。';
      status.hidden = false;
    };
    button.addEventListener('click', () => {
      ...
      if (!video.hasAttribute('src')) {
        const compact = matchMedia('(max-width: 780px), (pointer: coarse)').matches || navigator.connection?.saveData;
        video.src = compact && video.dataset.mobileSrc ? video.dataset.mobileSrc : video.dataset.src;
      }
      video.load();
      // Do not await a fetch: play must stay inside the original user gesture.
      const play = video.play();
      if (play) play.catch(fail);
      video.tabIndex = 0;
    ...
    video.addEventListener('waiting', () => {
      if (disposed) return;
      status.textContent = '影片載入中…';
      status.hidden = false;
    });
    video.addEventListener('playing', () => {
      status.hidden = true;
      dialogContent.querySelectorAll('video').forEach(other => {
...
function releaseCaseVideos() {
  dialogContent.querySelectorAll('[data-video-player]').forEach(player => player.releaseVideo?.());
}

document.addEventListener('visibilitychange', () => {
  if (document.hidden) dialogContent.querySelectorAll('video').forEach(video => video.pause());
});
